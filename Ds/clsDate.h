...
public:
	...
	static	string DateAndTime() {
		...
		// 1. تحديد الفترة قبل تحويل الساعة
		string Period = (Hours >= 12) ? "PM" : "AM";


		Hours = (Hours % 12 == 0) ? 12 : Hours % 12;

		// 3. تجميع السلسلة النصية بشكل منظم
		string Assemble = to_string(Day) + "/" + to_string(Month) + "/" + to_string(Year);
		Assemble += " - " + to_string(Hours) + ":" + to_string(Minutes) + ":" + to_string(Seconds);
		Assemble += " " + Period;
